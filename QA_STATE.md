...
E (təhlükəsizlik), C (ödəniş), D (e-poçt/şifrə bərpası) BİTDİ (F1–F21 düzəldildi; rate limit/DoS dərini və yarış halları K-da). SESSİYA DAYANDIRILDI (istifadəçi "saxla sabaha" dedi) — DAVAM: B-nin QALAN hissəsi. ARTIQ EDİLİB: OWNER ilə 18 admin səhifəsinin hamısı (dashboard…settings, account, /kitchen) masaüstündə yükləndi, konsol xətası/error boundary/üfüqi scroll YOX. QALIR: (1) mobil (375px) üzrə eyni gəzinti + üfüqi taşma, (2) MANAGER/WAITER/KITCHEN rolları (müvəqqəti hesab `qa.tmp.rbac.*` — ƏVVƏL dəftərə yaz; qadağan URL-lər `/admin/staff` vs. Navigate ilə yönləndirməli), (3) formalar (məhsul/kateqoriya/masa/promo/işçi yaratma-redaktə-silmə, validasiya, boş vəziyyət), (4) real-time (yeni sifariş → admin/mətbəx canlı), (5) səhifələmə/axtarış/filtr, (6) əlçatanlıq. Brauzer tabı: admin çıxış edildi, viewport desktop-a qaytarıldı; backend 4000 və vite 5174 işləyir. Əvvəlki plan: real brauzerdə (Browser pane, `http://localhost:5174/admin`, real admin `admin@qrmenu.local` defolt şifrə ilə OWNER; digər rollar üçün `qa.tmp.rbac.*` müvəqqəti hesablar dəftərə yazıldıqdan sonra): hər səhifə üçün yüklənmə, boş/xəta vəziyyəti, formalar (validasiya), siyahı/səhifələmə, real-time (socket) yeniləmə, rol üzrə menyu/düymə görünüşü (WAITER/KITCHEN-ə qadağan olan səhifələr), konsol xətaları, mobil görünüş; sonra A (müştəri saytı) → A (müştəri saytı) → F → G → H → K (300 sifariş, `LOADTEST-` işarəsi) → I → J. Skriptlər: `qa/rbac-matrix.js`, `qa/probes.js` (yüksək limitli ikinci nüsxə üçün `PORT=4001 EMAIL_FLOW_LIMIT=100000 node src/server.js`, sonra `QA_BASE=http://localhost:4001`).
