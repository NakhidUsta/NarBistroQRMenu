...
E bölməsi BİTDİ (rol matrisi, injection/fuzz, mass assignment, IDOR, fayl yükləmə, socket, məlumat sızması, F5 HSTS/CSP real brauzerdə təsdiqləndi); yalnız rate limit/DoS dərin yoxlaması və yarış halları K (yük testi) ilə birlikdə qalıb. İNDİ: **C (ödəniş sistemi)** — `PAYMENT_PROVIDER=test` ilə: nağd/kartla-masada/onlayn axınlar, webhook imzası (saxta/təkrar/dəyişdirilmiş məbləğ), bir sifarişə iki dəfə ödəniş, refund, gecikmiş ödəniş vs. ləğv yarışı, hold müddəti, gəlir hesabatı; sonra D (e-poçt) → D (e-poçt) → B (admin panel, brauzerdə hər rol) → A (müştəri saytı) → F → G → H → K (300 sifariş, `LOADTEST-` işarəsi) → I → J. Skriptlər: `qa/rbac-matrix.js`, `qa/probes.js` (yüksək limitli ikinci nüsxə üçün `PORT=4001 EMAIL_FLOW_LIMIT=100000 node src/server.js`, sonra `QA_BASE=http://localhost:4001`).
