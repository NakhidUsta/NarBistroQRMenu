...
// Bir səhifədə neçə kateqoriya: telefon 2, planşet 4, kompüter 6 ("Hamısı" həmişə sabit qalır və sayılmır)
const tabsPerPage = () => (typeof window === 'undefined' ? 6 : window.innerWidth >= 1024 ? 6 : window.innerWidth >= 768 ? 4 : 2)
...
function CategoryTabs({ categories, activeId, onSelect }) {
  ...
  return (
    // Telefonda: yuxarıda ‹ 1/7 › sətri, altında tab-lar (tab-ın alt xətti bölücü xəttin üstündə qalır); kompüterdə hamısı bir sətirdə
    <div className="flex flex-wrap md:flex-nowrap items-center md:items-start justify-center md:justify-start gap-2 md:gap-2 px-5 md:px-8">
      ...
      <div className="order-last basis-full mt-2 md:mt-0 md:order-none md:basis-auto flex gap-6 md:gap-9 overflow-x-auto md:flex-1 min-w-0 pb-1 no-scrollbar" style={{ scrollbarWidth: 'none' }}>
        {visible.map((cat) => {
